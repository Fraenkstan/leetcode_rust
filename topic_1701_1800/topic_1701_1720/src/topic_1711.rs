...
pub fn count_pairs(deliciousness: Vec<i32>) -> i32 {
    ...
    for i in 0..(delicious_vec.len()) {
        for j in if *delicious_map.get(delicious_vec[i]).unwrap() > 1 { i..delicious_vec.len() }
        else { (i + 1)..delicious_vec.len() } {
            if (0..22).into_iter().any(|k| 2_i32.pow(k) == (delicious_vec[i] + delicious_vec[j]) as i32) {
                if i == j {
                    let count = delicious_map.get(delicious_vec[i]).unwrap();
                    ans += count * (count - 1) / 2;
                }
                else {
                    ans += delicious_map.get(delicious_vec[i]).unwrap() * delicious_map.get(delicious_vec[j]).unwrap();
                }
            }
        }
    }
